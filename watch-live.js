...
function updateSpcImages() {
  ...
  if (cat) {
    cat.src =
      `https://www.spc.noaa.gov/products/outlook/day1otlk.gif?sv=${stamp}`;

    cat.onload = () =>
      setText('spcOutlookStatus','Official SPC Day 1 outlook loaded.');

    cat.onerror = () =>
      setText('spcOutlookStatus','SPC outlook image is temporarily unavailable.');
  }

  if (tor) {
    tor.src =
      `https://www.spc.noaa.gov/products/outlook/day1probotlk_torn.gif?sv=${stamp}`;

    tor.onload = () =>
      setText('spcTornadoStatus','Official SPC tornado outlook loaded.');

    tor.onerror = () =>
      setText('spcTornadoStatus','SPC tornado outlook image is temporarily unavailable.');
  }
}
