...
function severeRundown(ctx, alert) {
  ...
  if (broadcastLoopCount === 0) {
    segments.push(
      `This is StormVector breaking weather coverage. A ${event} is in effect for ${area}.`
    );
  } else {
    segments.push(
      `I'm continuing StormVector coverage of the ${event} for ${area}.`
    );
  }

  if (movement) {
    segments.push(
      `The National Weather Service says the storm is moving ${movement.direction} at ${movement.speed} miles per hour.`
    );
  }

  if (expires) {
    segments.push(
      `The warning is currently scheduled to remain in effect until ${expires}.`
    );
  }
  ...
  /*
    Keep life-safety instructions in every severe rundown.
    This is intentionally repetitive during a warning.
  */
  segments.push(
